...
def main(args):
    if (args.ppg == None):
        ...
        print(
            f"Auto run : python whisper/inferencev2.py -w {args.wave} -p {args.ppg}")
        os.system(f"python whisper/inferencev2.py -w {args.wave} -p {args.ppg}")

    if (args.vec == None):
        args.vec = "svc_tmp.vec.npy"
        print(
            f"Auto run : python hubert/inferencev2.py -w {args.wave} -v {args.vec}")
        os.system(f"python hubert/inferencev2.py -w {args.wave} -v {args.vec}")
    ...
    # Find the index of the last slash from the end
    last_slash_index = spk_str.rfind('/')
    if last_slash_index != -1:
        # Perform a slicing operation to get the desired part
    ...
    output_filename = name_str + "_" + "rmvpepit_" +"large-v2ppg_" + model_str + "_" + sliced_part + "_"+ shift_str
